...
namespace UnityEditor.Tilemaps
{
    ...
    public class GridBrushEditorBase : Editor
    {
        ...
        {
            OnPaintSceneGUIInternal(gridLayout, brushTarget, position, tool, executing, icon, target.name);
        }
        ...
        internal static void OnPaintSceneGUIInternal(GridLayout gridLayout, GameObject brushTarget, BoundsInt position, GridBrushBase.Tool tool, bool executing, Texture2D icon = null, string name = null)
        {
            if (Event.current.type != EventType.Repaint)
                return;
            ...
            {
                if (GridBrushEditorPreferences.sceneViewTextEnable)
                {
                    GridEditorUtility.DrawTextLabel(gridLayout, position, name, GridBrushEditorPreferences.sceneViewTextSize);
                }
            }
        }
    }
