...
namespace UnityEditor.Tilemaps
{
    internal class GridBrushesDropdown : PopupWindowContent
    {
        class Styles
        {
            ...
            public Texture2D linkIcon = EditorGUIUtility.LoadIcon("Linked");
            public Texture2D selectedLinkIcon = EditorGUIUtility.LoadIcon("d_Linked");
        }
        ...
        public GridBrushesDropdown(Action<int, object> itemClickedCallback, float parentWidth)
        {
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            var linkedBrush = GridTargetBrushSync.instance.GetLinkedBrush(GridPaintingState.scenePaintTarget);
                ...
                for (int i = 0; i <= maxIndex; ++i)
                {
                    ...
                    switch (evt.type)
                    {
                        case EventType.Repaint:
                            ...
                            var brush = m_ItemProvider.GetItem(i) as GridBrushBase;
                            var hasTarget = brush != null && brush == linkedBrush;

                            const float iconSize = 16f;
                            const float iconPadding = 3f;
                            var name = m_ItemProvider.GetName(i);
                            var truncateWidth = hasTarget ? m_CachedWidth - iconSize - iconPadding : m_CachedWidth;
                            var tname = TextUtility.TruncateWithEllipsis(s_Styles.menuItem, name, truncateWidth);
                            s_Styles.menuItem.Draw(itemRect, GUIContent.Temp(tname), hover, false, i == selectedIndex, false);

                            if (hasTarget)
                            {
                                Rect iconRect = new Rect(itemRect.xMax - iconSize - iconPadding, itemRect.y, iconSize, iconSize);
                                GUI.DrawTexture(iconRect, hover ? s_Styles.selectedLinkIcon : s_Styles.linkIcon);
                            }
        ...
        protected Vector2 CalcSize()
        {
            ...
            if (m_CachedWidth < 0)
                m_CachedWidth = m_ParentWidth; // CalcWidth()); Palette names are truncated
