...
namespace UnityEditor.Tilemaps
{
    internal static class GridEditorUtility
    {
        ...
        public static GUIStyle s_TextStyle = new GUIStyle(EditorStyles.label);
        ...
        public static void DrawTextLabel(GridLayout gridLayout, BoundsInt area, string text, int fontSize = 12, float screenHorizontalOffset = 0f)
        {
            if (Event.current.type != EventType.Repaint)
                return;
            ...
            var localPosition = new Vector3Int(area.min.x, area.max.y, area.min.z);
            var worldPosition = gridLayout.CellToWorld(localPosition);
            switch (gridLayout.cellLayout)
            {
                case GridLayout.CellLayout.Hexagon:
                    worldPosition = gridLayout.LocalToWorld(gridLayout.CellToLocalInterpolated(area.min));
                    break;
            }

            fontSize = Mathf.Clamp(fontSize, 4, 64);

            Handles.BeginGUI();
            s_TextStyle.fontSize = fontSize;
            s_TextStyle.alignment = TextAnchor.UpperLeft;

            var content = new GUIContent(text);
            var size = s_TextStyle.CalcSize(content);
            var padding = new Vector2(4f, 2f);

            var screenPoint = HandleUtility.WorldToGUIPoint(worldPosition);
            screenPoint.x += EditorGUIUtility.PixelsToPoints(new Vector2(screenHorizontalOffset, 0)).x;
            screenPoint.y -= (size.y + padding.y * 2);

            var backgroundRect = new Rect(screenPoint.x, screenPoint.y, size.x + padding.x * 2f, size.y + padding.y * 2f);

            EditorGUI.DrawRect(backgroundRect, backgroundColor);
            DrawBox(backgroundRect, Color.rebeccaPurple);
            GUI.Label(new Rect(screenPoint.x + padding.x, screenPoint.y + padding.y, size.x, size.y), content, s_TextStyle);
            Handles.EndGUI();
        }
