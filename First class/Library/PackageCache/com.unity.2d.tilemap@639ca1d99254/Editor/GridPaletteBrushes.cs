...
namespace UnityEditor.Tilemaps
{
    internal class GridPaletteBrushes : ScriptableSingleton<GridPaletteBrushes>
    {
        ...
        private void RefreshBrushesCache()
        {
            ...
            var allBrushTypes = TypeCache.GetTypesDerivedFrom<GridBrushBase>();
            var brushTypes = new List<Type>();
            foreach (var type in allBrushTypes)
            {
                if (type != typeof(GridBrush))
                    brushTypes.Add(type);
            }
        ...
        public static string GetAssetPathFromBrush(GridBrushBase brush)
        {
            if (brush == null)
                return String.Empty;
            var path = AssetDatabase.GetAssetPath(brush);
            if (string.IsNullOrEmpty(path))
                path = GenerateGridBrushInstanceLibraryPath(brush.GetType());
            return path;
        }

        public static GridBrushBase GetBrushFromAssetPath(string assetPath)
        {
            foreach (var brush in brushes)
            {
                if (AssetDatabase.GetAssetPath(brush).Equals(assetPath))
                    return brush;
                if (GenerateGridBrushInstanceLibraryPath(brush.GetType()).Equals(assetPath))
                    return brush;
            }
            return null;
        }

        private static string GenerateGridBrushInstanceLibraryPath(Type brushType)
        {
